...
--if not storage then error("Storage not found") end
storage = "right"
...
function scanWorkRequests(mon, rs, chest)
    ...
    for index, item in ipairs(items) do
        item_array[item.name] = {amount = item.amount, nbt = json.encode(item.nbt)}
        if item.name == "domum_ornamentum:side_framed" then
            print("domum found")
        end
    end
    ...
    for w in pairs(workRequests) do
        ...
        nbtt = workRequests[w].items[1].nbt
        ...
        if string.find(item, "domum_ornamentum") then useRS = 0 end
        ...
        if name == "Fuel" then item = "minecraft:coal" end
        ...
        if useRS == 1 then
            provided = 0
            if item_array[item] and item_array[item].amount > 0 then
                provided = rs.exportItem({name=item, count=needed}, chest)
            end
            ...
            if provided < needed then
                if rs.isItemCrafting({name=item}) then
                    ...
                    if rs.isItemCraftable({name=item}) and rs.craftItem({name=item, count=needed}) then
                        color = colors.yellow
